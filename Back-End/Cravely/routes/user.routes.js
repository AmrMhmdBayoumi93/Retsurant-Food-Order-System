...
exports.getAllusers = async (req, res, next) => {
  try {
    const allUsers = await User.find().select("-password"); //  .select("-password")
    // to hide pass
...
exports.register = async (req, res, next) => {
  try {
    ...
    if (Checkuser) {
      return res
        .status(400)
        .json({ Message: "Email already exists please user different email" });
    }
    ...
    return res
      .status(201)
      .json({ Message: "User is created successfully", newUser, token });
  } catch (error) {
    next(error);
  }
};
